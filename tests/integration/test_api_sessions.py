...
def test_create_and_list_session(client: TestClient) -> None:
    fid = _make_fighter(client)
    r = client.post("/sessions", json={"fighter_id": fid, "source": "live_webcam"})
    assert r.status_code == 201
    sid = r.json()["id"]
...
def test_capture_status_for_idle_session(client: TestClient) -> None:
    fid = _make_fighter(client)
    sid = client.post("/sessions", json={"fighter_id": fid, "source": "live_webcam"}).json()["id"]
    r = client.get(f"/sessions/{sid}/capture/status")
    assert r.status_code == 200
    body = r.json()
...
def test_events_endpoint_empty_for_new_session(client: TestClient) -> None:
    fid = _make_fighter(client)
    sid = client.post("/sessions", json={"fighter_id": fid, "source": "live_webcam"}).json()["id"]
...
def test_upload_rejected_for_live_source(client: TestClient) -> None:
    fid = _make_fighter(client)
    sid = client.post("/sessions", json={"fighter_id": fid, "source": "live_webcam"}).json()["id"]
...
def test_upload_attaches_path(client: TestClient, tmp_path) -> None:  # type: ignore[no-untyped-def]
    fid = _make_fighter(client)
    sid = client.post("/sessions", json={"fighter_id": fid, "source": "uploaded_video"}).json()[
        "id"
    ]
