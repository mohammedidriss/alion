                ...
                if pose:
                    ...
                    for ev in events:
                        ...
                        if pose.world_landmarks:
                            ...
                            row = [
                                current_class, ev.hand, ev.velocity_ms,
                                ls.x, ls.y, ls.z,
                                rs.x, rs.y, rs.z,
                                lw.x, lw.y, lw.z,
                                rw.x, rw.y, rw.z
                            ...
                            dataset.append(row)
                            count += 1
                            print(f"✅ Recorded {current_class} ({count}/{target_count}) - {ev.velocity_ms:.1f} m/s")
                            
                    draw_pose(frame, pose)
                
                # Display instructions on screen
                cv2.putText(frame, f"Throw: {current_class.upper()} ({count}/{target_count})", 
